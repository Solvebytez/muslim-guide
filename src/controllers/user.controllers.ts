...
export const userLogin = asyncHandler(async (req: Request, res: Response) => {
  ...
  const user = await User.findOne({ email:(decoded as any).email, isVerified: true }).select(
    "+password"
  );
  if (!user) {
    throw new ValidationError("User not found");
...
export const userProfile = asyncHandler(async (req: RequestWithUser, res: Response) => {
...
export const adminLogin = asyncHandler(async (req: Request, res: Response) => {
  ...
  const accesstoken = jwt.sign( { _id: user._id, email, role: user.role },config.JWT_ACCESS_SECRET, {
    expiresIn: "1d",
  })

  const refreshToken = jwt.sign( { _id: user._id, email, role: user.role },config.JWT_REFRESH_SECRET, {
    expiresIn: "7d",
  })
...
export const refreshToken = asyncHandler(async (req: RequestWithUser, res: Response) => {
  ...
  const newAccessToken = jwt.sign( { _id: user._id, email: user.email, role: user.role },config.JWT_ACCESS_SECRET, {
    expiresIn: "1d",
  })
  ...
  apiSuccessResponse(res, "Login successful", httpCode.OK,{...user.toObject(),isl_admin_access_token: newAccessToken });
  
})


export const userLogout = asyncHandler(async (req: RequestWithUser, res: Response) => {
    const userId= req.user._id;
    const token =
